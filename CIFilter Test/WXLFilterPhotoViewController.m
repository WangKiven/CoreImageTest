...
- (void)viewDidLoad
{
    ...
    //使用GPU做图片渲染
    EAGLContext *_glContext = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2];
    if (!_glContext) {
        NSLog(@"Failed to create ES context");
    }
    self.context = [CIContext contextWithEAGLContext:_glContext];
    //使用CPU做图片渲染
//    self.context = [CIContext contextWithOptions:nil];
    ...
    NSLog(@"filter可能的属性及其属性的数据类型总和：%@",mdic);
